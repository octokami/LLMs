...
def xml_to_dataframe(xml_file, format='csv', output_file = 'trial_data', in_dir = './data', ACD = False):
    """
    ...
    df = pd.DataFrame(columns=['raw_text', 'aspectTerms'])
                ...
                for value in sentence:
                    ...
                    for opinion in value:
                        if ACD:
                            ...
                            completion.append(dict({'term':opinion.attrib['target'], 
                            'polarity':opinion.attrib['polarity']}))

            df = pd.concat([df,pd.DataFrame.from_dict({
                #'id': Review.attrib['rid'],
                'raw_text': (' ').join(prompt),
                'aspectTerms': list(completion),
                }, orient='index').T])
                            
    df.reset_index(inplace=True, drop=True)

    if format =='jsonl':
        df.to_json(os.path.join(in_dir, output_file+'.'+format), orient='records', lines=True)
    
    if format == 'csv':
        df.to_csv(os.path.join(in_dir, output_file+'.'+format), index=False) #, sep="\t")
